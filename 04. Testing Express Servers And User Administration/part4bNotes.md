...
## Refactoring Tests
- Our test coverage is lacking.
- Requests like `GET /api/notes/:id` and `DELETE /api/notes/:id` are not tested when request sent with invalid id.
- Grouping of tests can also be improved.
    - For better readability.
    - Place them in `describe` blocks.
```js
const supertest = require("supertest");
const mongoose = require("mongoose");
const helper = require("./test_helper");
...
describe("Addition of a new note", () => {
    test("Succeeds with valid data", async () => {
        ...
        const notesAtEnd = await helper.notesInDb();
        expect(notesAtEnd).toHaveLength(helper.initialNotes + 1);
...
```
- Test output is grouped according to the `describe` blocks.
- Still can improve.
    - But just move on.
- The method of testing the API by making HTTP requests and inspecting db with Mongoose is not the best way of doing API-level integration tests for server applications.
- No universal best way to write tests.
    - Depends on application being tested and resources available.
